...
public:
	...
	static FTransform Default_GetWaistOrientationAndPosition(FBPVRWaistTracking_Info & WaistTrackingInfo)
	{
		...
		*/
		...
		{
			DeviceTransform.ConcatenateRotation(FRotator(0, 0, 0).Quaternion());
			
			break;
		}
		case EBPVRWaistTrackingMode::VRWaist_Tracked_Rear:
		{
			DeviceTransform.ConcatenateRotation(FRotator(0, -180, 0).Quaternion());
			
			break;
		}
		case EBPVRWaistTrackingMode::VRWaist_Tracked_Left:
		{
			DeviceTransform.ConcatenateRotation(FRotator(0, 90, 0).Quaternion());
