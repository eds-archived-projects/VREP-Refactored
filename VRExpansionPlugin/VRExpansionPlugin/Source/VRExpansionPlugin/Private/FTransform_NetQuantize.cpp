...
bool FTransform_NetQuantize::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
  ...
  //FQuat    rRotation   ;
	...
	if (Ar.IsSaving())
	{
		...
		{
			bOutSuccess &= SerializePackedVector<100, 30>(rTranslation, Ar);   // Translation set to 2 decimal precision.
			bOutSuccess &= SerializePackedVector<100, 30>(rScale3D    , Ar);   // Scale set to 2 decimal precision, had it 1 but realized that I used two already even.
	...
	else // If loading
	{
		...
		this->SetComponents(rRotation.Quaternion(), rTranslation, rScale3D);   // Set it.
	}
