...
bool UVRPathFollowingComponent::HasReachedCurrentTarget(const FVector& CurrentLocation) const
{
	...
	// Or standing at target position don't use acceptance radius here, it has to be exact for moving near corners (2D test < 5% of agent radius).
	const float GoalRadius = 0.0f;
	const float GoalHalfHeight = 0.0f;
...
void UVRPathFollowingComponent::UpdatePathSegment()
{
	...
	if (bCanUpdateState && Status == EPathFollowingStatus::Moving)
	{
		...
		else if (HasReachedCurrentTarget(CurrentLocation))   // Check if current move segment is finished.
		{
			OnSegmentFinished ();
			SetNextMoveSegment();
		}
...
int32 UVRPathFollowingComponent::DetermineStartingPathPoint(const FNavigationPath* ConsideredPath) const
{
	...
	{
		// If we already have some info on where we were on previous path we can find out if there's a segment on new path we're currently at.
		if 
		(
		...
		)
		{
			...
			for (int32 PathPoint = 0; PathPoint < ConsideredPath->GetPathPoints().Num() - 1; ++PathPoint)
			{
				if 
				(
					ConsideredPath->GetPathPoints()[PathPoint    ].NodeRef == MoveSegmentStartRef &&
		...
		if (MovementComp && PickedPathPoint == INDEX_NONE)
		{
			if (ConsideredPath->GetPathPoints().Num() > 2)
			{
				...
				// Making this test in 2d to avoid situation where agent's Z location not being in "navmesh plane" would influence the result.
				...
				PickedPathPoint = 
					FMath::IsNearlyEqual(SqDistToFirstPoint, SqDistToSecondPoint) ?
