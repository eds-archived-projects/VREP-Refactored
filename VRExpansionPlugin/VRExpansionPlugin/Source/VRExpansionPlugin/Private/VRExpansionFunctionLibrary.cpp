...
void UVRExpansionFunctionLibrary::GetGripSlotInRangeByTypeName
(
	FName       SlotType          , 
	AActor*     Actor             , 
...
)
{
	...
	if (USceneComponent *rootComp = Actor->GetRootComponent())
	{
		...
		for (int socketIndex = 0; socketIndex < SocketNames.Num(); ++socketIndex)
		{
			if (SocketNames[socketIndex].ToString().Contains(GripIdentifier, ESearchCase::IgnoreCase, ESearchDir::FromStart))
			{
				float vecLen = FVector::DistSquared(RelativeWorldLocation, rootComp->GetSocketTransform(SocketNames[socketIndex], ERelativeTransformSpace::RTS_Component).GetLocation());
...
void UVRExpansionFunctionLibrary::GetGripSlotInRangeByTypeName_Component
(
	FName                SlotType          , 
...
)
{
	...
	{
		if (SocketNames[socketName].ToString().Contains(GripIdentifier, ESearchCase::IgnoreCase, ESearchDir::FromStart))
		{
			float vecLen = FVector::DistSquared(RelativeWorldLocation, Component->GetSocketTransform(SocketNames[socketName], ERelativeTransformSpace::RTS_Component).GetLocation());

			if (MaxRange >= vecLen && (ClosestSlotDistance < 0.0f || vecLen < ClosestSlotDistance))
			{
				ClosestSlotDistance = vecLen    ;
				bHadSlotInRange     = true      ;
				foundIndex          = socketName;
			}
		}
	}
...
EBPHMDDeviceType UVRExpansionFunctionLibrary::GetHMDType()
{
	if (GEngine && GEngine->XRSystem.IsValid())
	{
		...
		*/
		...
		if      (DeviceName == FName(TEXT("SimpleHMD")))    return EBPHMDDeviceType::DT_ES2GenericStereoMesh;
		else if (DeviceName == SteamVRSystemName       )    return EBPHMDDeviceType::DT_SteamVR             ;
		else if (DeviceName == OculusSystemName        )    return EBPHMDDeviceType::DT_OculusHMD           ;
		else if (DeviceName == PSVRSystemName          )    return EBPHMDDeviceType::DT_PSVR                ;
		else if (DeviceName == OSVRSystemName          )    return EBPHMDDeviceType::DT_SteamVR             ;
		else if (DeviceName == GoogleARCoreSystemName  )    return EBPHMDDeviceType::DT_GoogleARCore        ;
		else if (DeviceName == AppleARKitSystemName    )    return EBPHMDDeviceType::DT_AppleARKit          ;
		else if (DeviceName == GoogleVRHMDSystemName   )    return EBPHMDDeviceType::DT_GoogleVR            ;
	}
...
void UVRExpansionFunctionLibrary::NonAuthorityMinimumAreaRectangle
(
...
)
{
	...
	float          MinDotResultA, MinDotResultB, 
		           MaxDotResultA, MaxDotResultB ;
